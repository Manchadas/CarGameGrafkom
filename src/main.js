...
function addObstacleBody() {
  const numberOfObstacles = 5; // Number of obstacles to A
  const obstacleSize = 1; // Size of the obstacle (adjust as needed)
  const spacingBetweenObstacles = 15; // Spacing between each obstacle (adjust as needed)
  const roadWidth = 10; // Width of the road (adjust as needed)

  for (let i = 0; i < numberOfObstacles; i++) {
    // Generate random position along the road width and at different distances
    const randomX = (Math.random() - 0.5) * roadWidth; // Random position within the road width
    const randomZ = -(i + 1) * spacingBetweenObstacles; // Spaced along the road

    let obstacleShape = new CANNON.Box(new CANNON.Vec3(obstacleSize, obstacleSize, obstacleSize));
    obstacleBody = new CANNON.Body({ mass: 1 });
    obstacleBody.addShape(obstacleShape);
    obstacleBody.position.set(randomX, 5, randomZ); // Set the random position
...
function addObstacle() {
  ...
  for (let i = 0; i < obstaclesBodies.length; i++) {
    ...
    obstacleMesh.position.copy(obstaclesBodies[i].position); // Set obstacle position from bodies
...
function resetObstacles() {
  // Remove existing obstacle meshes from the scene
  for (let i = 0; i < obstaclesMeshes.length; i++) {
    scene.remove(obstaclesMeshes[i]);
  }

  // Clear arrays holding obstacle bodies and meshes
  obstaclesBodies = [];
  obstaclesMeshes = [];
  ...
  addObstacleBody();
  addObstacle();
  // Add logic here to position obstacles based on your requirements
}
...
// Find the reset button in the DOM
...
function updateScore() {
  ...
  console.log('Score:', score); // You can replace this with your logic to display the score
}

// Function to reset the car's position
function resetCar() {
  // Reset the car's position to the initial position
  cubeBody.position.set(0, 2, 0); // Adjust the initial position as needed
  cubeBody.velocity.set(0, 0, 0); // Reset the velocity
  cubeBody.angularVelocity.set(0, 0, 0); 
  cubeBody.quaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), Math.PI); 
  cubeThree.position.copy(cubeBody.position);
  cubeThree.quaternion.copy(cubeBody.quaternion);

  // Increment the score when the car is reset
  updateScore();
}
...
function movePlayer(){
  ...
  const strengthWS = 10000;
  const forceForward = new CANNON.Vec3(0, 0, strengthWS)
  if(keyboard[87]) cubeBody.applyLocalForce(forceForward);

  // down letter S
  const maxBrakingForce = 200;
  const strengthBrake = maxBrakingForce;
  const currentVelocity = cubeBody.velocity.clone();

  if (keyboard[83]) {
    // Calculate the braking force (opposite to current velocity)
    const forceBrake = currentVelocity.clone().negate().scale(strengthBrake);
    cubeBody.applyForce(forceBrake, cubeBody.position); // Apply the braking force
  }
